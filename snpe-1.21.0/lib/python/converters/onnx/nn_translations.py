...
class OnnxBatchNormalizationTranslation(OnnxTranslationBase):
    def __init__(self):
        ...
        self.index_method(SQUASH_BATCHNORM, self.squash_batchnorm)
    ...
    def extract_parameters(self, src_op, graph):
        params = extract_attributes(src_op,
                                    ...
                                    ('is_test','i',0),
                                    ('spatial','i',1))
        LOG_ASSERT(params.is_test, ERROR_BATCHNORM_TEST_ONLY)
    ...
    def squash_batchnorm(self, node, graph):
        input_buffer = graph.get_input_buffers(node)[0]
        prev = input_buffer.producer
        if prev.op.type == 'convolution':
            LOG_DEBUG(DEBUG_BATCHNORM_SQUASH,
                     node.op.name,
                     prev.op.name)

            # The Conv weights are not yet transposed as that happens in axes_to_snpe later,
...
class OnnxMaxRoiPoolTranslation(OnnxTranslationBase):
    ...
    def axes_to_snpe_order(self, node, graph):
        ...
        output_buf = graph.get_output_buffers(node)[0]
        LOG_ASSERT(output_buf.shape[0] == 1,
